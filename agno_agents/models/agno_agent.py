...
class AgnoAgent(models.Model):
    ...
    agent_role = fields.Text('Agent Role', required=True, 
    ...
    # Model configuration
    model_id = fields.Char('Model ID', required=True, default="qwen3:latest",
    ...
    api_key_env = fields.Char('API Key Environment Variable', 
                             default="API_KEY",
    ...
    # Database settings
    db_file = fields.Char('Database File', default="agent.db",
    ...
    status = fields.Selection([
        ...
        ('stopping', 'Stopping'),
        ('error', 'Error')
    ], default='stopped', string='Status', tracking=True)
    ...
    def _check_agent_name(self):
        for record in self:
            if not record.agent_name or len(record.agent_name.strip()) == 0:
...
'''
    ...
    def unlink(self):
        """Override unlink to stop agents before deletion"""
        for record in self:
            if record.is_active:
